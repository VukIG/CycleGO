...
import { Tent, Droplets, Coffee, MapPin, Filter, Star, Wrench } from 'lucide-react';
const allLocations = [{
  id: 1,
  ...
  id: 6,
  name: 'Bike Shop Valley',
  type: 'repair',
  distance: '10km',
  rating: 4.8,
  amenities: ['tools', 'parts'],
  icon: Wrench,
...
export function CampingView() {
  ...
  return <div className="h-full bg-[#0F1419] pt-20 px-4 overflow-y-auto pb-24">
      ...
      <div className="flex gap-3 overflow-x-auto pb-4 scrollbar-hide mb-4">
        {filters.map(f => <button key={f.id} onClick={() => setFilter(f.id)} className={`flex items-center gap-2 px-4 py-2 rounded-full border transition-colors whitespace-nowrap ${filter === f.id ? 'bg-emerald-500/20 border-emerald-500 text-emerald-400' : 'bg-slate-800/50 border-slate-700 text-slate-400'}`}>
            <f.icon size={14} />
      ...
      <div className="space-y-4">
        ...
        {filteredLocations.map((site, i) => <motion.div key={site.id} initial={{
        opacity: 0,
        y: 20
      }} animate={{
        opacity: 1,
        ...
        delay: i * 0.05
            ...
            <div className="flex gap-2 mt-3 pl-[52px]">
              {site.amenities.map(a => <span key={a} className="text-[10px] px-2 py-1 rounded bg-slate-700/50 text-slate-300 capitalize">
                  {a}
                </span>)}
            </div>
          </motion.div>)}
